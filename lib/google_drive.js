...
function GoogleDrive(profile, googleAuth) {
  ...
  var CACHE_DURATION_MIN = 10;  // 10 Minutes
  var _blockCache = {};
  ...
  self.getFileBytesByRange = function(item, offset, len, buf, cb) {
    ...
    for (var i=startBlock; i<=endBlock; i++) {
      _getFileBlock(item, i, function(err, fileBlock){
        ...
        //console.log(' gotBlock', item.id, fileBlock.number, fileBlock.start, fileBlock.end);
        ...
        //console.log(' copyBlock', item.id, fileBlock.number, targetStart, sourceStart, sourceEnd);
    ...
    for (var i=endBlock+1; i<=Math.min(endBlock+5,lastBlock); i++)
    {
  ...
  function _releaseOldBlocksFromCache() {
    console.log('Checking file/block cache');
    ...
    for (var fileKey in _blockCache) {
      ...
      for (var blockKey in fileCache) {
        ...
        if (fileBlock.lastAccessed < new Date().addMinutes(0 - CACHE_DURATION_MIN)) {
          console.log('Removing block', blockKey, 'of file', fileKey, 'from cache');
          delete fileCache[blockKey];
          if (Object.keys(fileCache).length === 0) {
            console.log('Removing file', fileKey, 'from cache');
    ...
    googleAuth.auth.request(request_opts, function(err, body, response) {
      ...
      if (response.statusCode === 403 && !dontRetry) {
        ...
        if (response.statusCode !== 206) {
          console.log('downloadFileBytesByRange', item.id, start, end, Date.now() - startT + ' ms.', '[ERROR]');
          return cb(new Error('Error. HttpStatusCode=' + response.statusCode));
        }

        console.log('downloadFileBytesByRange', item.id, start, end, Date.now() - startT + ' ms.');
        cb(null, body);
      }
    });
  }
