...
function GoogleDriveFS(profile, googleDrive) {
  ...
  function _init(cb) {
    ...
    profile.info('Filesystem mounted at \'%s\'',_config.mount_point);
  ...
  function _destroy(cb) {
    if (_mounted) {
      profile.info('Filesystem stopped (mount point: \'%s\')', _config.mount_point);
    }
  ...
  function _getattr(path, cb) {
    ...
    switch (item.type) {
      case 'folder':
        ...
        stat.nlink = item.childItems.length + 2; // TODO... verify this.
